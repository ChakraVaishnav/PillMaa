...
import { REPEAT_TYPES } from '../../src/constants';
...
const schema = z.object({
  ...
  reminderTime:   z.string().min(1, 'Required'),
...
export default function EditReminderScreen() {
  ...
  const reminder = useReminderStore((s) => s.reminders.find((r) => r.id === id));
  ...
  const { control, handleSubmit, watch, setValue, reset, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: {
      ...
      repeatType:     (reminder?.repeatType ?? 'DAILY') as RepeatType,
      repeatDays:     reminder?.repeatDays ?? [],
      snoozeCount:    reminder?.snoozeCount ?? 3,
      snoozeInterval: reminder?.snoozeInterval ?? 5,
    },
  });
  ...
  const handleDelete = () => {
    Alert.alert(
      'Delete Reminder',
      `Would you like to delete "${reminder?.medicineName}" only for today, or forever?`,
      [
        {
          text: 'Cancel',
          style: 'cancel',
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              console.warn('[Edit] Delete failed:', error);
            }
          },
        },
      ],
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          ...
          <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
              ...
              <Card style={styles.formSection}>
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange, onBlur } }) => (
                    <Input label="Medicine Name" value={value} onChangeText={onChange} onBlur={onBlur} error={errors.medicineName?.message} />
                  )}
                />
                <Controller
                  control={control}
            ...
            <Animated.View entering={FadeInDown.delay(100)}>
              <Card style={styles.formSection}>
                ...
                <TouchableOpacity style={styles.timePicker} onPress={() => setShowTimePicker(true)}>
                  ...
                  <Text style={[styles.timePickerValue, { color: theme.primary }]}>{formatReminderTime(reminderTime)}</Text>
              ...
              <Card style={styles.formSection}>
                ...
                <View style={styles.repeatRow}>
                  {REPEAT_TYPES.map((type) => (
                    <TouchableOpacity
                      key={type.key}
                      style={[styles.repeatChip, repeatType === type.key && { backgroundColor: theme.primaryLight, borderColor: theme.primary }]}
                      onPress={() => setValue('repeatType', type.key as RepeatType)}
                    >
                      <Feather
                        name={type.icon as any}
                        size={18}
                        color={repeatType === type.key ? theme.primary : Colors.textSecondary}
                      />
                      <Text style={[styles.repeatChipLabel, repeatType === type.key && { color: theme.primary, fontFamily: Typography.fontFamily.semibold }]}>
                        {type.label}
                      </Text>
                    </TouchableOpacity>
                  ))}
                  ...
                  <View style={{ paddingTop: 8 }}>
                    ...
                    <Controller
                      ...
                      render={({ field: { value, onChange } }) => (
                        <DaySelector selected={value as RepeatDay[]} onChange={onChange as any} />
                      )}
                    />
                  </View>
                )}
            ...
            <Animated.View entering={FadeInDown.delay(250)} style={styles.submitSection}>
              <Button title="Update Reminder" onPress={handleSubmit(onSubmit)} isLoading={isUpdating} fullWidth size="lg" style={{ backgroundColor: theme.primary }} />
            </Animated.View>
          </ScrollView>
        </KeyboardAvoidingView>
      </SafeAreaView>
      <TimePickerModal visible={showTimePicker} value={reminderTime} onConfirm={(t) => setValue('reminderTime', t)} onClose={() => setShowTimePicker(false)} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  backIcon: { fontFamily: Typography.fontFamily.medium, fontSize: 16, color: Colors.textSecondary },
  ...
  fieldLabel: { fontFamily: Typography.fontFamily.medium, fontSize: Typography.fontSize.sm, color: Colors.textSecondary, marginBottom: 8 },
  timePicker: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: Colors.neutral[50], borderRadius: 14, padding: 16,
    borderWidth: 1.5, borderColor: Colors.border,
  },
  timePickerLabel: { fontFamily: Typography.fontFamily.medium, fontSize: Typography.fontSize.base, color: Colors.textSecondary, flex: 1 },
  timePickerValue: { fontFamily: Typography.fontFamily.bold, fontSize: Typography.fontSize.xl, color: Colors.primary[600], marginRight: 8 },
  timePickerChevron: { fontFamily: Typography.fontFamily.bold, fontSize: Typography.fontSize.xl, color: Colors.textTertiary },
  repeatRow: { gap: 8 },
  repeatChip: {
  ...
  repeatChipSelected: { backgroundColor: '#EAF5EF', borderColor: '#1a7a4a' },
  repeatChipLabel: { fontFamily: Typography.fontFamily.medium, fontSize: Typography.fontSize.base, color: Colors.textSecondary },
  repeatChipLabelSelected: { color: '#1a7a4a', fontFamily: Typography.fontFamily.semibold },
  snoozeRow: { flexDirection: 'row', gap: 16 },
  snoozeItem: { flex: 1 },
  stepper: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    backgroundColor: Colors.neutral[50], borderRadius: 12, padding: 8,
    borderWidth: 1.5, borderColor: Colors.border,
  },
  stepperBtn: {
  ...
  stepperBtnText: { fontFamily: Typography.fontFamily.bold, fontSize: Typography.fontSize.lg, color: '#1a7a4a' },
  stepperValue: { fontFamily: Typography.fontFamily.bold, fontSize: Typography.fontSize.base, color: Colors.textPrimary, minWidth: 36, textAlign: 'center' },
